...
| `rescue_building_xl` | 84 × 40 m | `-41,-19,41,19` |

값이 월드보다 크면 벽 밖 경계가 후보로 잡혀 **수색이 영영 안 끝난다.**

RViz 는 `ugv_multi.rviz` 를 쓴다. 로봇별로 색이 다른 경로가 나온다.

로봇은 1~3대까지 된다. 다만 **권장은 2대다** — 3대는 어느 맵에서도 이득이
확인되지 않았다([docs/SCALING.md](docs/SCALING.md)).
...
근거는 [docs/EXPERIMENTS.md](docs/EXPERIMENTS.md) 3.2절에 있다.
