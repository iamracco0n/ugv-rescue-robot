...
while [ "$elapsed" -lt "$DURATION" ]; do
  ...
  # '전원 발견' 보고에서 끊으면 안 된다(기본 끔).
  # 그 보고는 등록 '수' 만 보므로 오탐이 섞이면 잘못 뜬다. 실제로
  # 오탐 1건이 수를 채워 6/7 상태에서 전원 발견이 떴고, 여기서 런을
  # 끊는 바람에 남은 진짜 조난자를 더 찾을 기회가 사라졌다
  # (측정이 결과를 바꾼 셈이다). 로봇 자체는 그 뒤로도 보충 수색을
  # 계속하므로, 채점 런은 시간이 찰 때까지 그냥 둔다.
  # 속도만 빠르게 재고 싶을 때만 STOP_ON_ALL_FOUND=1 로 켠다.
  if [ "${STOP_ON_ALL_FOUND:-0}" = 1 ] && grep -q '전원 발견' "$LOG" 2>/dev/null; then
