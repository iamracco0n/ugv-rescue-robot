...
지금 이 완화는 기본값이 꺼짐이다
--------------------------------
동선 운을 없앤 집중 시험(3머신 48런)에서 이득이 확인되지 않아
(base 13/24 vs 완화 15/24, p 약 0.55) 기본값을 6 / 0.50 으로 되돌렸다.
이 테스트는 '켰을 때 규칙이 의도대로 도는가' 를 잠근다 — 기본값과는
무관하게, 나중에 파라미터로 켤 때 동작을 보장하기 위한 것이다.
"""
import ast
import math
import os
