...
MIN_PERSIST = 1.0
CONSIST_R   = 0.60
TRACK_GAP   = 3.0    # 이보다 오래 끊기면 다른 후보로 본다
...
CASES = [
    ...
    # ↓ 포탑 스윕으로 탐지가 끊기는 실제 상황.
    #   '연속' 을 요구했더니 진짜 조난자가 7/7 → 2/7 로 무너졌다.
    #   기각 67건 중 66건이 '시간 부족' 이었다.
    ('진짜 사람 — 포탑 스윕으로 2초 끊겼다 다시 보임',
     [(0.0, 12.0, 10.0), (0.3, 12.05, 10.0), (2.3, 12.0, 10.05),
      (2.6, 12.02, 9.98)], True),
    ('진짜 사람 — 끊김 2.9초(허용 한계 안)',
     [(0.0, 8.5, -14.0), (0.3, 8.52, -14.0), (3.2, 8.5, -13.98)], True),
]
