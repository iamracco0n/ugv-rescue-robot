...
3. **1대 → 2대는 확실한 이득** — 10분 시점 발견 인원 2배(3 → 6명), 두 머신·두 맵에서
   재현 ([6.4절](#64-로봇-대수-확장))
4. **2대 → 3대는 이득이 없다** — 28×20 부터 168×40 까지 네 맵 어디서도 3대가 앞서지
   못했고, XXL(168×40)에서는 두 머신 모두 모든 시점에서 뒤졌다. 원인은 Nav2 경로 탐색
   실패 루프다 ([6.4절](#64-로봇-대수-확장) · [6.5절](#65-더-큰-맵에서는-어떤가--xxl))
...
| `rescue_building_xxl` | 168 × 40 m | 34 | — | 대수 확장 재확인 (제한 90분) |
...
**표 9.** XL 월드(84 × 40 m, 조난자 13명, 제한 45분) 시점별 발견 인원 — 메인 (유효 85런)
...
> **주 — XL 에서는 재현되지 않았다.** 에일리언웨어에서 2대 16런 · 3대 15런을 쟀는데
> 그쪽에서는 **둘이 동률**이었다(45분 평균 11.9 대 11.9). 그래서 XL 결과만으로는
> 잠정 결론이었다. 아래 XXL 실험이 그 판정을 뒤집는다.

### 6.5 더 큰 맵에서는 어떤가 — XXL

"84 × 40 이 3대에게 좁아서 이득이 안 났나" 를 가르려고 면적을 두 배로 키웠다.
`rescue_building_xxl`(168 × 40 m, 방 32개, 조난자 34명, 제한 90분)이다.

**표 12.** XXL 월드 시점별 발견 인원 (평균)
...
맵을 두 배로 키우면 3대가 통하리라는 예상은 빗나갔다. 오히려 XL 보다 격차가 크다 —
XL 은 초반(10 · 20분)이 동률이었는데 XXL 은 10분에 이미 갈린다(9.0 대 6.1).

**그리고 이번에는 두 머신이 일치한다.** XL 에서 재현이 안 돼 잠정으로 남겼던 판정이
XXL 에서 확인됐다. 6.4절의 기전(planner 실패 루프)이 맵이 커질수록 더 크게 작용하는
것으로 보인다 — 복도가 길어지면 한 로봇이 실패 루프에 빠졌을 때 비는 구역도 넓어진다.

**권장 구성은 2대다.** 28 × 20 · 56 × 40 · 84 × 40 · 168 × 40 어느 맵에서도 3대가
이득이라는 근거가 없다.
