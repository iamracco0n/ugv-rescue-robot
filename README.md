...
| **30분 안에 완주** | **4/7 (57%)** | **82~90%** (유효 50런 x 2회) |
| 조난자별 발견률 | 7명 전원 7/7 | lying_s2 84~94% · 나머지 96~100% |
| 유령 검출 | 1건 / 7런 | 2건 · 1건 / 각 50런 |

**같은 30분을 주면 2대가 크게 앞섭니다.** 시간으로 봐도 약 1.4배 빠릅니다.

2대는 **유효 50런짜리 검증을 두 번**(각 머신 25런씩) 돌렸고 82% 와 90% 가
나왔습니다. 신뢰구간이 겹쳐 서로 모순되지 않지만, 두 검증 사이에 계측
로깅이 추가돼 코드가 완전히 같지는 않으므로 **합치지 않고 따로 적습니다.**
한 번만 쟀으면 어느 쪽이든 과신이었을 폭입니다.
...
못 찾은 런은 대부분 같은 사람(남서 방 바닥에 누운 조난자)을 놓칩니다.
그 방은 남쪽 다섯 방 중 유일하게 내부 칸막이가 있어, 문이 15m 깊이 주머니의
구석에 붙어 있습니다.

처음에는 탐사 알고리즘의 결함으로 보고 여덟 가지를 시험했는데 하나도 이득이
없었습니다. 제한을 45분으로 늘려 보니 **기본 설정이 그 조난자를 11런 전부
찾았습니다**(30분에서는 82%). 로봇은 그 구석에 갈 줄 압니다 — 30분 안에 못
갈 뿐입니다.
...
**86% 는 알고리즘의 한계가 아니라 30분이라는 예산의 값입니다.** 더 올리려면
탐사 정책이 아니라 시간이나 로봇 대수를 늘리는 쪽입니다.
그 방은 남쪽 다섯 방 중 유일하게 내부 칸막이가 있어, 문이 15m 깊이 주머니의
