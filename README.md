...
정답 좌표가 알려진 재난 건물 맵에서 **유효 런 300회 이상**으로 측정했다.
...
5. **그 실패 루프를 고치자 3대 열세가 거의 사라졌다** — XL 45분 격차 −0.64 → −0.17명
   (조건당 약 50런). 다만 뒤집히지는 않아 **권장 구성은 그대로 2대다**
   ([6.6절](#66-실패-루프를-고친-뒤))
...
6.4절이 지목한 원인(Nav2 경로 탐색 실패 루프)을 고치고 XL 에서 같은 조건으로 다시 쟀다.
2대 52런 · 3대 49런으로 기준선(각 26런)의 두 배다.

**무엇이 문제였나.** 순찰기는 목표를 `/goal_pose` 토픽으로 던져 Nav2 의 결과를 못 받았다.
Nav2 가 복구행동을 다 쓰고 손을 든 뒤에도 자기 제한시간(최대 150초)이 다 흐를 때까지 그
목표를 붙들고 서 있었다. 박힘 감지도 이 구간은 못 잡는다 — Nav2 가 속도를 안 내고 있으니
'일부러 선 것' 으로 보고 넘긴다.
...
45분 런에서 3대가 서 있기만 하는 시간이 9.9분이었다. 고친 뒤 회당 대기는 **1초 이하**다.

**표 14.** 3대 — 수정 전(31런) 대 수정 후(49런), 평균 ± 표준오차
...
3대는 세 시점이 유의하게 올랐다. 2대는 20분 시점 하나만 올랐다(+0.88, p<0.01) — 되찾을
대기 시간이 3대에 훨씬 많았으니 방향이 맞다.
...
**3대 열세는 거의 사라졌지만 뒤집히지는 않았다.** 45분에서 2대가 여전히 앞서고(p=0.03)
그것만 유의하다. 10 · 20 · 30분은 세 시점 다 못 가른다.

네 시점을 한꺼번에 검정했으므로 p=0.03 은 다중비교를 감안하면 경계값이고, 격차 0.17명은
13명 중 1.3 % 라 실용적으로는 미미하다.

**결론은 바뀌지 않는다 — 권장 구성은 2대다.** 어느 시점에서도 3대가 유의하게 앞서지
못했다. 다만 "3대는 쓰면 손해" 라고까지 말할 근거는 이제 약하다.

<sub>수정 전후 비교는 `tools/compare_runs.py`, 대기 시간은 `tools/dead_time.py` 로 낸다.
XXL 은 수정 후에 다시 재지 않았으므로 6.5절은 수정 전 측정이다.</sub>
...
3. **실패 루프의 절반만 고쳤다**(표 10 · [6.6절](#66-실패-루프를-고친-뒤)). Nav2 가 포기한
   **뒤** 서 있던 시간은 없앴지만, planner 가 애초에 왜 실패하는지 — 로봇끼리 서로를
   코스트맵/지도에 굽는 문제 — 는 그대로다. 남은 방향은 로봇끼리 코스트맵에서 서로를
   지우는 쪽이다.
4. **RGB/Depth 동기화가 굶으면 로봇이 조용히 눈먼다.** 카메라 토픽은 정상 발행 중인데 두
   스트림 속도가 어긋나면(실측 8.7 Hz 대 11.2 Hz) 동기화 구독이 짝을 못 맞춰 검출이 0 이
   된다. 그 로봇은 주행은 멀쩡히 하면서 담당 구역 조난자를 통째로 놓친다. `yolo_pose_node`
   의 눈멂 감시는 '프레임은 오는데 결과가 없는' 것만 보므로 **이 경우를 못 잡는다.**
   배경 CPU 부하와 같이 나타났다.
5. 커버리지 완주 후의 회차 완료 보고는 아직 실측하지 못했다.
6. 직선 접근이 막혔을 때의 우회 접근은 단위 테스트로만 검증됐다(실측 발동 0회).
7. 실로봇 통합은 진행 중이다. 열화상 카메라는 시뮬 전용이며 실기에는 아직 없다.
