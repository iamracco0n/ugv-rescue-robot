...
2,499줄. 프론티어(라이다 미탐사 경계)와 시각 커버리지(카메라 미관측 영역)를
...
| 구독 | `navigate_to_pose/_action/status` | `GoalStatusArray` — **Nav2 의 목표 처리 결과** |
...
| `nav_abort_react` | `true` | Nav2 가 포기하면 제한시간을 안 기다린다 |
| `nav_abort_grace_s` | `3.0` | 목표 발행 직후 이 시간 안의 통보는 무시 |
| `nav_abort_max_streak` | `3` | 연속 이 횟수 포기하면 후진 탈출 |
...
목표는 `goal_pose` **토픽**으로 나간다. 액션 클라이언트가 아니라서 Nav2 의
결과를 못 받았고, Nav2 가 복구행동(backup · spin · wait)을 다 쓰고 손을 든
뒤에도 순찰기는 위 제한시간이 다 흐를 때까지 그 목표를 붙들고 서 있었다.
박힘 감지도 이 구간은 못 잡는다 — Nav2 가 속도를 안 내고 있으므로
`stuck_decision` 이 '일부러 선 것' 으로 보고 넘긴다(복구행동 중에는 옳은
판단이다).

액션 서버는 상태를 평범한 토픽으로도 내보낸다. 그래서 액션 클라이언트로
갈아타지 않고 `navigate_to_pose/_action/status` 를 구독해 결과를 듣는다.
QoS 는 액션 상태 토픽의 규정값(depth 1 · reliable · transient_local)을 그대로
맞춰야 한다. 안 맞으면 구독은 되는데 메시지가 하나도 안 온다.

**함정: `ABORTED` 는 '포기' 만 뜻하지 않는다.** Nav2 는 우리가 목표를 갈아끼울
때도 밀려난 목표를 `ABORTED` 로 끝낸다(`CANCELED` 가 아니다). 순찰기는 도착 후
정지 · 조난자 확인 · 순찰 재개마다 목표를 새로 던지므로 그런 `ABORTED` 가 흔하다.
실측으로 한 로봇은 `bt_navigator` 가 `Aborting handle` 을 한 번도 안 찍었는데도
(진짜 포기 0회) 상태 토픽에는 여러 건이 올라왔고, 그대로 받으면 멀쩡한 목표를
버린다.

가르는 규칙은 **포기한 목표가 목록에서 가장 최근 목표일 때만 진짜로 본다** 는
것이다. 우리가 밀어낸 것이라면 방금 던진 더 새로운 목표가 뒤에 있고, Nav2 가
정말 손을 든 것이라면 그보다 새로운 목표는 없다.

받은 뒤의 처리는 두 갈래다. 판정은 순수 함수 `abort_decision` 으로 떼어
`tools/test_abort_decision.py` 에서 검사한다.

| 상황 | 처리 |
|---|---|
| 한두 번 포기 | 목표 탓으로 보고 그 프론티어를 방문 처리, 다른 곳으로 |
| `nav_abort_max_streak` 회 연속 | 로봇 탓으로 보고 후진 탈출(다른 목표를 골라도 똑같이 실패한다) |

연속을 세는 이유는, 로봇이 팽창영역 안에 들어가 어디로도 경로가 안 나올 때
재선정만 반복하면 프론티어 목록을 몇 초 만에 태우고 로봇은 그 자리에 남기
때문이다.
...
*[README](README.md), 실험 기록은 [docs/EXPERIMENTS.md](docs/EXPERIMENTS.md) 와*
*[docs/SCALING.md](docs/SCALING.md) 에 있다.*
